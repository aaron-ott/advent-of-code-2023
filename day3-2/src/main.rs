...
fn check_number(character_map: &Vec<Vec<char>>, row: usize, col: usize) -> (bool, usize, u32, usize, usize) {
    ...
    for i in i_start..=i_end {
        for j in j_start..=j_end {
            if !character_map[i][j].is_alphanumeric() && character_map[i][j] != '.' {
                return (true, length, value, i, j);
            }
        }
    }

    return (false, length, 0, 0, 0);
}
...
fn main() {
    ...
    let mut value_vec: Vec<(u32, usize, usize)> = Vec::new();
    ...
    for row in 0..character_map.len() {
        ...
        while col < character_map[0].len() {
            if character_map[row][col].is_numeric() {
                let (is_target, length, value, row_index, col_index) = check_number(&character_map, row, col);
                if is_target {
                    value_vec.push((value, row_index, col_index));
                    col += length;
    ...
    for (i, (value, row, col)) in value_vec.iter().enumerate() {
        // println!("Base Element: {} {} {} {}", i, value, row, col);
        for (_, (value2, row2, col2)) in value_vec[(i + 1)..].iter().enumerate() {
            // println!("Gear 1: {} {} {} {}; Gear 2: {} {} {} {}", i, value, row, col, j, value2, row2, col2);
